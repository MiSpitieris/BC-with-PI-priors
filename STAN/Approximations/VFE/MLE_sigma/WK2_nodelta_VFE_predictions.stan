// predictions for VFE nodelta model
functions {
  ...
  vector Pgp_pred_rng(vector tP,
    ...
    vector[n_pred] f_pred;
    {
      ...
      // K_uu_inv = inverse(K_uu + diag_matrix(rep_vector(1e-6,m)));
      // Lambda = K_uu_inv + K_uu_inv*K_uf*Sigma_inv*K_fu*K_uu_inv;
      // mm = inverse(Lambda + diag_matrix(rep_vector(1e-6,m)))*K_uu_inv*K_uf*Sigma_inv*y;
      // mu = K_su*K_uu_inv*mm;
...
generated quantities {
  ...
  for(n in 1:N_samples) {
    f_P = Pgp_pred_rng(tP, tI, yP, yI, tP_pred[1:nP_pred], alpha[n], rho[n], R[n], C[n], sigmaP, sigmaI, zP, zI);
    for(np in 1:nP_pred) {
  ...
  for(n in 1:N_samples) {
    f_I = Igp_pred_rng(tP, tI, yP, yI, tI_pred[1:nI_pred], alpha[n], rho[n], R[n], C[n], sigmaP, sigmaI, zP, zI);
    for(np in 1:nI_pred) {
