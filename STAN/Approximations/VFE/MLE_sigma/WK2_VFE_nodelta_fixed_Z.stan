...
functions {
  ...
  matrix K_PP(vector tP_i,
    ...
    K[1:nP_i, 1:nP_j] = K[1:nP_i, 1:nP_j];
    return(K);
   }
  ...
  matrix K_II(vector tI_i, 
    ...
    for (i in 1:nI_i){
          for (j in 1:nI_j){
            ...
            + C2 * (0.2e1 * rho2_ * exp(-pow(tI_i[i] - tI_j[j], 0.2e1) * rho2_) 
            - 0.4e1 * pow(tI_i[i] - tI_j[j], 0.2e1) * rho4_ * exp(-pow(tI_i[i] - tI_j[j], 0.2e1) * rho2_));
            K[i, j] = alpha2 * K[i, j];
          }
    }
  return(K);
...
model {
  ...
  B = diag_matrix(rep_vector(1,m)) + A*Sigma_inv*A';
  ...
  // trace term
  trc = -0.5*(trace(Sigma_inv*K_ff)+ trace(A*Sigma_inv*A')); 
