...
generated quantities {
  ...
  for(n in 1:N_samples) {
    f_P = Pgp_pred_rng(tP, tI, yP, yI, tP_pred[1:nP_pred], alpha[n], rho[n], rho_d[n], alpha_d[n], R[n], C[n], sigmaP[n], sigmaI[n], zP, zI);
    for(np in 1:nP_pred) {
  ...
  for(n in 1:N_samples) {
    f_I = Igp_pred_rng(tP, tI, yP, yI, tI_pred[1:nI_pred], alpha[n], rho[n], rho_d[n], alpha_d[n], R[n], C[n], sigmaP[n], sigmaI[n], zP, zI);
    for(np in 1:nI_pred) {
