functions {
  ...
  matrix K_heat(matrix xtU, matrix xtF,
    ...
    // KU_Bias
    for (i in 1:(nU-1)){
      KUB[i,i] = pow(sigma_UB, 0.2e1);
      for (j in (i+1):nU){
        KUB[i,j] = Kuu_fn(xtU[i,1], xtU[j,1], xtU[i,2], xtU[j,2], lx_UB, lt_UB);
        KUB[i,j] = pow(sigma_UB, 0.2e1) * KUB[i,j];
        KUB[j,i] = KUB[i,j];
      }
      KUB[nU,nU] = pow(sigma_UB, 0.2e1);
    }
    K[1:nU, 1:nU] = K[1:nU, 1:nU]+KUB[1:nU,1:nU];
  ...
  matrix KUB_pred(matrix xtU,
    ...
    for (i in 1:nU_pred){
      for (j in 1:nU){
        KU[i,j] = Kuu_fn(xtU_pred[i,1], xtU[j,1], xtU_pred[i,2], xtU[j,2], lx_UB, lt_UB);
        KU[i,j] = pow(sigma_UB, 0.2e1) * KU[i,j];
      }
    }
    return KU;
  } 
  
  matrix KFB_pred(matrix xtF,
                  matrix xtF_pred,
                  real lx_FB, real lt_FB,
                  real sigma_FB
  ){
    int nF = rows(xtF);
    int nF_pred = rows(xtF_pred);
    ...
    for (i in 1:nF_pred){
      for (j in 1:nF){
        KF[i,j] = Kuu_fn(xtF_pred[i,1], xtF[j,1], xtF_pred[i,2], xtF[j,2], lx_FB, lt_FB);
        KF[i,j] = pow(sigma_FB, 0.2e1) * KF[i,j];
      }
    }
    return KF;
  }  
  ...
  vector Ugp_pred_rng(matrix xtU,
    ...
    vector[nU_pred] f2;
    {
      ...
      L_K =  K_heat(xtU, xtF, lx, lt, sigma, sigmaU, sigmaF, alpha, lx_UB, lt_UB, sigma_UB);
      ...
      cov_f2 = KU_pred(xtU_pred, xtU_pred, lx, lt, sigma) 
      - v_pred' * v_pred;
...
parameters {
  ...
  // U-Bias parameters
  real<lower=0,upper=1> lx_UB;
  real<lower=0,upper=10> lt_UB;
  real<lower=0,upper=100> sigma_UB;
  // physical parameters
...
model {
  // Chol. of PI kernel
  matrix[nU + nF, nU + nF] L_K = K_heat(xtU, xtF, lx, lt, sigma, sigmaU, sigmaF, alpha, lx_UB, lt_UB, sigma_UB);
  // mean vector
  vector[nU + nF] mu = mu_fn(mu_heat, nU, nF);
  // priors
  lx ~ normal(0,1.0/3);
  lt ~ normal(0,1.0);
  lx_UB ~ normal(0,1.0/3);
  lt_UB ~ normal(0,1.0);
  sigma ~ normal(0,5);
  sigma_UB ~ normal(0,5);
...
generated quantities {
  ...
  f_U = Ugp_pred_rng(xtU, xtF, yU, yF, xtU_pred, sigma, lx, lt, alpha, sigmaU, sigmaF, lx_UB, lt_UB, sigma_UB);
  for (n1 in 1:nU_pred)
    y_U[n1] = normal_rng(f_U[n1], sigmaU);
  f_F = Fgp_pred_rng(xtU, xtF, yU, yF, xtF_pred, sigma, lx, lt, alpha, sigmaU, sigmaF, lx_UB, lt_UB, sigma_UB);
  for (n2 in 1:nF_pred)
