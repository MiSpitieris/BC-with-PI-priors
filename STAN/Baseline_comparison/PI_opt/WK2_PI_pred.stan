...
generated quantities {
  ...
  f_P=Pgp_pred_rng(tP, tI, yP, yI, tP_pred, alpha, rho, R, C, sigmaP, sigmaI);
  for(np in 1:nP_pred) {
    y_P[np] = normal_rng(f_P[np], sigmaP);
  }  

  f_I=Igp_pred_rng(tP, tI, yP, yI, tI_pred, alpha, rho, R, C, sigmaP, sigmaI);
  for(np in 1:nI_pred) {
    y_I[np] = normal_rng(f_I[np], sigmaI);
  }
}
