...
model {
  // Chol. of PI kernel
  matrix[nP + nI, nP + nI] L_K = K_wk2(tP, tI, l, sigma, sigmaP, sigmaQ, R, C);
  // mean vector
...
generated quantities {
  ...
  f_P = Pgp_pred_rng(tP, tI, yP, yI, tP_pred, sigma, l, R, C, sigmaP, sigmaQ);
  for (n1 in 1:nP_pred)
    y_P[n1] = normal_rng(f_P[n1], sigmaP);
  
  f_I = Igp_pred_rng(tP, tI, yP, yI, tI_pred, sigma, l, R, C, sigmaP, sigmaQ);
  for (n2 in 1:nI_pred)
