functions {
  ...
  matrix K_wk2(matrix tP, 
    ...
    for (i in 1:(nP-1)){
      ...
      for (j in (i+1):nP){
        a=tP[i,1]; 
        b=tP[j,1];
        KB[i,j] = exp(-0.2e1 * pow(sin(pi() * (a - b) * p), 0.2e1) * pow(l_delta, -0.2e1));
  ...
  vector Pgp_pred_rng(matrix tP,
    ...
    vector[nP_pred] f2;
    {
      ...
      L_K =  K_wk2(tP, tI, l, sigma, p, sigmaP, sigmaQ, l_delta, sigma_delta, R, C);
      K_div_y = mdivide_left_tri_low(L_K, y);
      K_div_y = mdivide_right_tri_low(K_div_y', L_K)';

      qT_p[1:nP_pred, 1:nP] = KP_pred(tP, tP_pred, l, sigma, p)+KP_Bias(tP, tP_pred, l_delta, sigma_delta,p);
      qT_p[1:nP_pred, (nP+1):N] = KPI_pred(tI, tP_pred, l, sigma, p, R, C);
      f2_mu = (qT_p * K_div_y);
      v_pred = mdivide_left_tri_low(L_K, qT_p');
      cov_f2 = KP_pred(tP_pred, tP_pred, l, sigma, p)+KP_Bias(tP_pred, tP_pred, l_delta, sigma_delta,p) - v_pred' * v_pred;
...
model {
  // Chol. of PI kernel
  matrix[nP + nI, nP + nI] L_K = K_wk2(tP, tI, l, sigma, p, sigmaP, sigmaQ,l_delta, sigma_delta, R, C);
  // mean vector
...
generated quantities {
  ...
  f_P = Pgp_pred_rng(tP, tI, yP, yI, tP_pred, l, sigma, l_delta, sigma_delta, p, R, C, sigmaP, sigmaQ);
  for (n1 in 1:nP_pred)
    y_P[n1] = normal_rng(f_P[n1], sigmaP);

  f_I = Igp_pred_rng(tP, tI, yP, yI, tI_pred, l, l_delta, sigma_delta, sigma, p, R, C, sigmaP, sigmaQ);
  for (n2 in 1:nI_pred)
