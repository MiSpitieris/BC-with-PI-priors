functions {
  ...
  vector Pgp_pred_rng(matrix tP,
    ...
    vector[nP_pred] f2;
    {
      ...
      L_K =  K_wk2(tP, tI, l, sigma, p, sigmaP, sigmaQ, R, C, yDia, yI);
...
model {
  // Chol. of PI kernel
  matrix[nP + nI, nP + nI] L_K = K_wk2(tP, tI, l, sigma, p, sigmaP, sigmaQ, R, C, yDia, yI);
  // mean vector
...
generated quantities {
  ...
  f_P = Pgp_pred_rng(tP, tI, yP, yI, tP_pred, l, sigma, p, R, C, sigmaP, sigmaQ, yDia);
  for (n1 in 1:nP_pred)
    y_P[n1] = normal_rng(f_P[n1], sigmaP);

  f_I = Igp_pred_rng(tP, tI, yP, yI, tI_pred, l, sigma, p, R, C, sigmaP, sigmaQ, yDia);
  for (n2 in 1:nI_pred)
