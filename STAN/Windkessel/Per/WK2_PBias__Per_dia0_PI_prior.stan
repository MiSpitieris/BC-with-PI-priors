functions {
  ...
  matrix K_wk2(matrix tP, 
    ...
    for (i in 1:(nP-1)){
      ...
      for (j in (i+1):nP){
        a=tP[i,1]; 
        b=tP[j,1];
        KB[i,j] = exp(-0.2e1 * pow(sin(pi() * (a - b) * p), 0.2e1) * pow(lBP, -0.2e1));
   ...
   // KI
    for (i in 1:(nI-1)){
     ...
     if(yI[i]==yDia) K[nP + i, nP +i] = K[nP + i, nP +i] + 1e-8;
     for (j in (i+1):nI){
      a=tI[i,1];
      b=tI[j,1];
      K[nP + i, nP +j] = 
     ...
     if(yI[nI]==yDia) K[nP + nI, nP +nI] = K[nP + nI, nP +nI] + 1e-8;
     }
  ...
  vector Pgp_pred_rng(matrix tP,
    ...
    vector[nP_pred] f2;
    {
      ...
      L_K =  K_wk2(tP, tI, l, sigma, p, sigmaP, sigmaQ, lBP, sigma_delta, R, C, yDia, yI);
      K_div_y = mdivide_left_tri_low(L_K, y);
      K_div_y = mdivide_right_tri_low(K_div_y', L_K)';

      qT_p[1:nP_pred, 1:nP] = KP_pred(tP, tP_pred, l, sigma, p)+KP_Bias(tP, tP_pred, lBP, sigma_delta,p);
      qT_p[1:nP_pred, (nP+1):N] = KPI_pred(tI, tP_pred, l, sigma, p, R, C);
      f2_mu = (qT_p * K_div_y);
      v_pred = mdivide_left_tri_low(L_K, qT_p');
      cov_f2 = KP_pred(tP_pred, tP_pred, l, sigma, p)+KP_Bias(tP_pred, tP_pred, lBP, sigma_delta,p) - v_pred' * v_pred;
...
model {
  // Chol. of PI kernel
  matrix[nP + nI, nP + nI] L_K = K_wk2(tP, tI, l, sigma, p, sigmaP, sigmaQ,lBP, sigma_delta, R, C, yDia, yI);
  // mean vector
...
generated quantities {
  ...
  f_P = Pgp_pred_rng(tP, tI, yP, yI, tP_pred, l, sigma, lBP, sigma_delta, p, R, C, sigmaP, sigmaQ, yDia);
  for (n1 in 1:nP_pred)
    y_P[n1] = normal_rng(f_P[n1], sigmaP);

  f_I = Igp_pred_rng(tP, tI, yP, yI, tI_pred, l, lBP, sigma_delta, sigma, p, R, C, sigmaP, sigmaQ, yDia);
  for (n2 in 1:nI_pred)
